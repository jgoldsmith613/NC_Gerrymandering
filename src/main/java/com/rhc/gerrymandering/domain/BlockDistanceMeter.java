...
public class BlockDistanceMeter implements NearbyDistanceMeter<Block, Block> {
	...
	public double getNearbyDistance(Block origin, Block destination) {
	    double distance =  Math.sqrt(Math.pow(Math.abs(origin.getLatitude() - destination.getLatitude()),2) +  Math.pow(Math.abs(origin.getLongitude() - destination.getLongitude()),2));
		/*count++;
		if (count % 1000 == 0) {
